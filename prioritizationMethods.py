class RICE:
    ...
    def __init__(self, reach: float, impact: float, confidence: float, effort: float):
        ...
        reach: Сколько людей или сегментов затронет проект.
        impact: Насколько сильно повлияет проект (оценка от 1 до 5).
        confidence: Уверенность в оценках reach и impact (процент от 0 до 100).
        effort: Количество усилий (в человеко-месяцах или других единицах), необходимых для реализации проекта.
        """
        self.reach = reach
        self.impact = impact
...
class ICE:
    ...
    def __init__(self, impact: float, confidence: float, effort: float):
        ...
        impact: Насколько сильно повлияет проект (оценка от 1 до 10).
        confidence: Уверенность в оценках impact (процент от 0 до 100).
        effort: Количество усилий (в человеко-месяцах или других единицах), необходимых для реализации проекта.
        """
        self.impact = impact
        self.confidence = confidence / 100  # Преобразуем процент в десятичную дробь
...
class Kano:
    ...
    def add_feature(self, feature_name: str, customer_satisfaction: int, functionality: int):
        ...
        feature_name: Название функциональности.
        customer_satisfaction: Оценка удовлетворения пользователей (от -1 до 1, где -1 — недовольны, 0 — индифферентно, 1 — довольны).
        functionality: Оценка функциональности (от -1 до 1, где -1 — отрицательно, 0 — нейтрально, 1 — положительно).
        """
